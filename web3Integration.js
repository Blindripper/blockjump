...
<head>
    ...
    color: #FFFFFF;
    padding: 20px 0 0 0; /* Add some top padding */
}
...
#leftSidebar {
    ...
    border-radius: 8px;
    box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
    margin-right: 10px; /* Reduced from 200px */
    margin-left: 10px;
}
...
function updatePowerupTimerBar(type, timeLeft, duration) {
    ...
    try {
        showBlockchainWaitMessage("Starting game on Etherlink...");
...
async function submitHighscore(name) {
    try {
        showBlockchainWaitMessage("Waiting for Etherlink...");
...
function showClaimPrizeButton() {
    ...
    document.body.appendChild(claimButton);
        ...
        function showBlockchainWaitMessage(message = "Waiting for Etherlink...") {
    const waitMessage = document.createElement('div');
    waitMessage.id = 'blockchain-wait-message';
    ...
    waitMessage.style.backgroundColor = 'rgba(0, 0, 0, 0.8)';
    waitMessage.style.color = '#3FE1B0';
    waitMessage.style.padding = '20px';
    waitMessage.style.borderRadius = '10px';
    waitMessage.style.zIndex = '1000';
    waitMessage.style.fontFamily = 'Orbitron, sans-serif';
...
document.getElementById('buyTriesBtn').addEventListener('click', async () => {
    try {
        showBlockchainWaitMessage("Purchasing game tries on Etherlink...");
